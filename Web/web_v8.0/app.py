...
def index():
    ...
    data = db.app_feedback.find()
    feedback_data = (data['rating'], data['feedback_text'])
...
def calculate_red_alert_zones():
    ...
    try:
        ...
        for pin_code in latest_pin_codes:
            latitude, longitude = get_coordinates_from_pin(pin_code)
            if latitude is not None and longitude is not None:
                red_alert_zones.append({'latitude': latitude, 'longitude': longitude, 'pin_code': pin_code})
        ...
        # Add count to red_alert_zones
        for zone in red_alert_zones:
            zone['count'] = pin_code_counts.get(zone['pin_code'], 0)
...
def process_accidents_data():
    try:
        ...
        for entry in new_entries:
            location = entry.get('Location', '')
            if location:
                cities = location.split(',')[-2].strip()
                cities=cities.split()[-2]
                all_cities.append(cities)

        print(all_cities)

        city_counts = Counter(all_cities)
        filtered_city_counts = {state: count for state, count in city_counts.items() if count > 5}
        print(city_counts)
...
def hospital_recieved(user_username, username, password):
    ...
    feedback_data = db.hospital_feedback.find_one({'ref_id': data['ref_id']})
    ...
    if days_difference is None or days_difference > 30:
        db.hospital_feedback.delete_one({'ref_id': data['ref_id']})
...
def hospital_feedback(user_username, username, password):
    ...
    if request.method == 'POST':
        ...
        db.hospital_feedback.insert_one(feedback_data)
        db.hospital_accepted.delete_one({'user_username': user_username})
        return redirect(url_for('hospital_notification',username=username,password=password))
...
def calculate(username, password, user_username):
    if request.method == "POST":
        ...
        if result and payment_method:
            ...
            feedback_data = db.ambulance_feedback.find_one({'ref_id': user_data['ref_id']})
            if feedback_data is None:
                days_difference = None
            else:
                previous_date = datetime.strptime(feedback_data['date'], "%Y-%m-%d")
                days_difference = (current_date - previous_date).days
            if days_difference is None or days_difference > 30:
                db.ambulance_feedback.delete_one({'ref_id': user_data['ref_id']})
...
def ambulance_feedback(user_username, username, password):
    ...
    if request.method == 'POST':
        ...
        db.ambulance_feedback.insert_one(feedback_data)
        db.ambulance_accepted.delete_one({'user_username': user_username})
        return redirect(url_for('ambulance_notification',username=username,password=password))
