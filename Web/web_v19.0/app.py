...
def getwellsoon(username,password):
    ...
    if msg_flag == 0:
        ...
        try:
            ...
            ambu_data1 = db.ambulance_accepted.find_one({"user_username": username})
            # print("yoyo", ambu_data1)
            hosp_data1 = db.hospital_accepted.find_one({"user_username": username})
            if ambu_data1 is None and hosp_data1 is None:
                raise TypeError('Data not found anywhere')
            elif ambu_data1 is not None and hosp_data1 is None:
                raise ValueError('Data not found in hospital table')
            ambu_data2 = db.ambulance_reg.find_one({"uid": ambu_data1['ref_id']})
            # print("yoyo" + ambu_data2)
            hosp_data2 = db.hospital_reg.find_one({"uid": hosp_data1['ref_id']})
            flash(f"Please wait, Your ambulance will be arriving soon<br>"
            f"Ambulance Details:--<br>"
            f"Driver Name: {ambu_data2['driverName']}<br>"
            f"Car Number: {ambu_data2['carNumber']}<br>"
            f"Phone Number: {ambu_data2['phone']}<br><br>"
            f"Hospital Details:--<br>"
            f"Hospital Name: {hosp_data2['hospitalName']}")
            ...
            f"Phone Number: {ambu_data2['phone']}<br><br>"
            f"Hospital Details:--<br>"
            f"Waiting for Hospital to accept request")
